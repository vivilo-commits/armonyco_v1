...
- **Institutional Audit Trail**: Replacing volatile UI indicators (alerts/toasts) with printable PDF-style documents (`GovernanceAuditModal`) elevates the product from a dashboard to a high-compliance operational tool.
- **Dynamic Report ID**: Generating unique crypto-style identifiers for report sessions creates an institutional "State of Truth" that builds user trust.
...
- **Print Media Visibility**: When using `visibility: hidden` on the `body` to isolate a modal for printing, you **must** use `#target, #target * { visibility: visible }` to ensure all nested components (cards, badges, text) inherit visibility. Standard CSS inheritance can sometimes fail for deeply nested flex/grid children in print mode.
- **Background Printing**: Modern browsers require `-webkit-print-color-adjust: exact` to render gradients and dark cards (like the Revenue section) in PDF exports.
